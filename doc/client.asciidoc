...
TIP: Using http://kcat.strangesoft.net/openal.html[OpenAL Soft] implementation
of OpenAL is recommended.
...
con_scale::
    ...
    Default value is 0 (automatically scale depending on current display
    resolution). Set to 1 to disable scaling.
...
cl_demoformat::
    Specifies format for recording demos.
        - 0 — original vanilla Quake II format
        - 2 — extension for demos with prediction
...
ui_scale::
    ...
    value is 0 (automatically scale depending on current display resolution).
